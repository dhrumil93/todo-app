...
This file is an _informal_ document, named "Architectural Decision Record" because I'm fond of the name and it gives it that extra level of sophistication.
...
_Visualize_ and _realize_ your vision with Task-Together, a task management app that will help you reach your goals.
...
- Next.js 14 App Router is my new favorite way to create front-end applications.
  ...
  - I wanted to incorporate its latest features such as: _partial prerendering_ and _server actions_.
...
Explain why services such as Clerk is not a viable option for Task-Together in the long-term due to its high per-user cost. Compare Clerk's pricing with other authentication providers and suggest alternatives.
...
```sh
npx create-next-app@latest Task-Together
```
...
export default function Home() {
  return <div className="text-sky-500">Task-Together</div>;
}
```
...
- A call to action (try Task-Together for free today)

```sh
Task-Together is a kanban-style productivity app that helps you turn your vision into reality. Plan, prioritize, and execute your goals with boards, lists, and cards. Task-Together your tasks with visionary kanban boards. Try Task-Together for free today.
```

May even mention a catchy slogan such as "_Task-Together your tasks with visionary kanban boards_". As we want to draw the connection between visonary (i.e., (especially of a person) thinking about or planning the future with imagination or wisdom) and visual representation of the tasks.
...
export const metadata: Metadata = {
  title: "Task-Together",
  description: `Task-Together is a productivity app that helps you
  turn your vision into reality. Plan, prioritize, and execute your goals
  with boards, lists, and cards. Task-Together your tasks with visionary kanban
  boards. Try Task-Together for free today.`,
};
...
export const siteConfig = {
  name: "Task-Together",
  description: `Task-Together is a kanban-style productivity app that helps you
  turn your vision into reality. Plan, prioritize, and execute your goals
  with boards, lists, and cards. Task-Together your tasks with visionary kanban
  boards. Try Task-Together for free today.`,
};
...
- `title`: an object that specifies the title of your page. It has two properties:
  - `default`: the default title of your page, which is the name of your app (`Task-Together`) from the `siteConfig` object that you imported.
  - `template`: a template string that can be used to generate dynamic titles based on the current route. It uses `%s` as a placeholder for the route-specific title, and appends the app name after a `|` character. For example, if the route-specific title is `Home`, the template will produce `Home | Task-Together`.
    ...
    <div className="flex items-center justify-center flex-col">
      <div className="flex items-center justify-center flex-col">
        <div>
          ...
          Achieve more with <strong>
            <em>Task-Together</em>
          </strong>, the ultimate task management app
        </div>
      </div>
    </div>
  );
...
Going to brainstorm some ideas of some promotional text I could use here. Task-Together allows for: project management, task management, brainstorming, resource hub, meetings and onboarding.
  ...
  return (
    <div className="flex items-center justify-center flex-col">
      ...
      <div className="text-sm md:text-xl text-neutral-400 mt-4 max-w-xs md:max-w-2xl text-center mx-auto">
        Fight mediocrity with Task-Together. With just boards, lists and cards
        you can get a clear overview of your tasks. Then you can plan,
        prioritize, and execute your goals with confidence. You can drag and
        drop tasks, add labels and due dates, attach files and comments, and
        more.
      </div>
    </div>
  );
  ...
  return (
    <div className="flex items-center justify-center flex-col">
      ...
      <Button className="mt-6" size="lg" asChild>
        <Link href="/sign-up">Try Task-Together for free</Link>
      </Button>
    </div>
  );
}
  ...
  return (
    <div className='flex items-center justify-center flex-col'>
      ...
      <div className={cn(
        ...
        Fight mediocrity with Task-Together. With just boards, lists and cards you can
...
We have the decision to either have individual users and organizations or just organizations. Task-Together will be for both individuals and organizations so that more will benefit.

Allow for both personal & organizational use

This commit changes the app logic and UI to enable Task-Together to be used for both personal and organizational purposes. The user can now switch between different modes and access different features depending on their needs.
  ...
  return (
    <nav className="flex items-center fixed px-4 z-10 top-0 w-full h-14 border-b shadow-sm bg-white">
      ...
      <div className="flex items-center gap-x-4">
        ...
        <Button size="sm" className="rounded-sm py-1.5 px-2 h-auto">
...
export default function Sidebar({ storageKey = "sidebarState" }: SidebarProps) {
...
```tsx
const [open, setOpen] = useLocalStorage<Record<string, any>>(storageKey, {});
```
...
export default function Sidebar({ storageKey = "sidebarState" }: SidebarProps) {
  ...
  return (
    <>
      ...
      <Accordion type="multiple" defaultValue={prevAccordionValue}></Accordion>
    </>
  );
}
```
...
export default function Sidebar({ storageKey = "sidebarState" }: SidebarProps) {
  ...
  // Use the Object.keys method to get the keys of the open state object
  // Use the reduce method to filter out the keys that have a false value
  const prevAccordionValue: string[] = Object.keys(open).reduce(
...
- The state has _one_ property called `bears`, which is a number that indicates how many bears are in the store.
- The actions are _two_ functions: `increasePopulation`, `removeAllBears`.
...
Arrow functions can have either an _expression_ body or the usual _block body_.
...
- Use the `create` function to create the custom hook `useMobileSidebar` with _one_ property `isOpen` and _two_ actions `onOpen` and `onClose`
    ...
    <>
      ...
      <Sheet open={isOpen} onOpenChange={onClose}>
        <SheetContent side="left" className="p-2 pt-10">
          <Sidebar storageKey="mobileSidebarState" />
        </SheetContent>
      </Sheet>
    </>
  );
...
export default function SkeletonSidebar() {
  return <div>SkeletonSidebar</div>;
}
```
...
export default function Board({ id, title }: BoardProps) {
  ...
  return (
    <form className="flex items-center gap-x-2">
      <p>{title}</p>
      <p>{id}</p>
      <Button type="submit" variant="default" size="sm" onClick={handleUpdate}>
        Update
      </Button>
      <Button
        type="submit"
...
export default function BoardFormInput({ errors }: BoardFormInputProps) {
  return (
    <div className="flex flex-col space-y-2">
      <Input
        id="title"
...
function createServerActionEffect<T, U, V>(
  input: T,
  schema: z.Schema<U>,
  handler: (output: U) => Promise<V>
);
```
    ...
    if (!validation.success) {
      ...
      return {
        fieldErrors: validation.error.flatten()
          .fieldErrors as FieldErrors<InputType>,
      };
    }
...
type ServerAction<InputType, OutputType> = (
  data: InputType
) => Promise<ActionState<InputType, OutputType>>;
...
export const useServerAction = <InputType, OutputType>(
  ...
  const [fieldErrors, setFieldErrors] = useState<
    FieldErrors<InputType> | undefined
  >(undefined);
...
export default function BoardForm() {
  const { executeServerAction, fieldErrors } = useServerAction(createBoard, {
    ...
    onSuccess: (data) => {
      console.log(data, "Successfully created Board!");
    },
  });
...
export default function BoardForm() {
  ...
  return (
    <form action={onSubmit}>
      <FormInput errors={fieldErrors} id="title" label="Board Title" />
      <FormSubmitButton size="default" variant="default" className="p-1">
        Save
      </FormSubmitButton>
    </form>
  );
...
export default function BoardCreationButton() {
  return (
    <Button className="relative flex flex-col items-center h-full w-full rounded-sm aspect-video bg-muted gap-y-1 justify-center transition hover:opacity-75">
      <p className="text-sm">Create new board</p>
      <span className="text-xs">15 remaining</span>
    </Button>
  );
}
```
...
export default function BoardCreationButton() {
  return (
    <Button className="relative flex flex-col items-center h-full w-full rounded-sm aspect-video bg-muted gap-y-1 justify-center transition hover:opacity-75">
      <p className="text-sm">Create new board</p>
      <span className="text-xs">{freeBoards} remaining</span>
      <BoardTooltip
        sideOffset={40}
        description={`
...
export default function BoardTooltip({ children }: BoardTooltipProps) {
  return <div>{children}</div>;
}
```
...
export default function FormPopover({ children }: FormPopoverProps) {
  return (
    <div>
      FormPopover
      {children}
    ...
    <Popover>
      ...
      >
        ...
        <form className="space-y-4">
          ...
          <FormSubmitButton size="default" variant="default" className="w-full">
            Create
          </FormSubmitButton>
        </form>
      </PopoverContent>
...
We are now in the page for "Apply for production". If you are going to make your application live then you can run this but for Task-Together, which is currently in development, we won't apply for production.
...
- Use the NEXT*PUBLIC* prefix to expose the access key to the browser
...
export default function FormSelector({ id, errors }: FormSelectorProps) {
  ...
  return (
    <Popover>
      ...
      >
        ...
        <form action={onSubmit} className="space-y-4">
          <div className="space-y-4">
            <FormSelector id="image" errors={fieldErrors} />
...
**Warning: migrate reset is a development command and should _never_ be used in a production environment.**
...
function startCase(input: string): string {
  ...
  const startCasedWords = words.map(
...
When we return a `title` using `generateMetadata` in anywhere else besides the root layout, it passes in the new title and is interpolated in the variable `%s` while appending the `| ${siteConfigName}`. So the output would be: "Organization Name | Task-Together".
...
export default function BoardOptions({ id }: BoardOptionsProps) {
  const { executeServerAction, isLoading } = useServerAction(deleteBoard, {
    onError: (error) => {
      toast.error(error);
    },
  });
...
export default function ListForm() {
  return (
    <ListWrapper>
      <button className="flex items-center w-full rounded-md p-3 font-medium text-sm bg-white/80 hover:bg-white/50 transition">
...
export default function ListHeader({ data }: ListHeaderProps) {
  return (
    <div className="flex pt-2 px-2 text-sm font-semibold justify-between items-start gap-x-2">
      {/* ListHeader display... */}
      <ListOptions data={data} handleAddCardToList={() => {}} />
    </div>
  );
}
```
    ...
    <Popover>
      ...
      <PopoverContent align="start" side="bottom" className="px-0 pt-3 pb-3">
        ...
        <form>
          <input hidden id="id" name="id" value={data.id} />
          <input hidden id="boardId" name="boardId" value={data.boardId} />
          <FormSubmitButton>Copy list</FormSubmitButton>
        </form>
        <Separator />
        <form>
          <input hidden id="id" name="id" value={data.id} />
          <input hidden id="boardId" name="boardId" value={data.boardId} />
          <FormSubmitButton>Delete list</FormSubmitButton>
        </form>
      </PopoverContent>
    </Popover>
  );
...
```tsx
const { executeServerAction: executeCopyServerAction } = useServerAction(
  copyList,
  {
    onSuccess(data) {
...
export default function ListItem({ data, index }: ListItemProps) {
  ...
  return (
    <li className="h-full w-72 shrink-0 select-none">
      <div className="w-full rounded-md bg-[#f1f2f4] shadow-md pb-2">
        <ListHeader data={data} handleAddCardToList={enableEditing} />
      </div>
    </li>
  );
}
...
export default function ListHeader({
  ...
  return (
    <div className="flex pt-2 px-2 text-sm font-semibold justify-between items-start gap-x-2">
      {/* ... */}
      <ListOptions data={data} handleAddCardToList={handleAddCardToList} />
    </div>
  );
}
```
...
export default function CardItem({ data, index }: CardItemProps) {
  return (
    <div className="py-2 px-3 bg-white text-sm shadow-sm rounded-md truncate border-2 border-transparent hover:border-black">
      {data.title}
    </div>
  );
}
...
2. Render an actual button
   ...
   export default function CardItem({ data, index }: CardItemProps) {
     return (
       <button className="py-2 px-3 bg-white text-sm shadow-sm rounded-md truncate border-2 border-transparent hover:border-black">
         {data.title}
       </button>
     );
   }
...
function onDragEnd(result: any) {
  ...
  if (type === "list") {
    ...
    const items = reorder(orderedListData, source.index, destination.index).map(
...
export const updateListOrder = createServerAction(
  UpdateListOrder,
  performAction
);
```
...
export default function CardModal() {
  return (
    <Dialog>
      <DialogContent>CardModal</DialogContent>
    </Dialog>
  );
}
```
...
-[TanStack Query | Overview](https://tanstack.com/query/latest/docs/framework/react/overview) -[TanStack Query | Quick start](https://tanstack.com/query/latest/docs/framework/react/quick-start)
...
export default function Header({ data }: HeaderProps) {
  ...
  return (
    <div>
      ...
      <div>
        <form>
          <FormInput id="title" defaultValue={title} />
        </form>
      </div>
    </div>
  );
...
export default function Header({ data }: HeaderProps) {
  const { executeServerAction: executeUpdateCard } =
...
```tsx
const { executeServerAction: executeUpdateCard, fieldErrors } = useServerAction(
  updateCard,
  {
    onSuccess(data) {
      // Invalidate the relevant query in the query cache
...
```tsx
export default function Actions({ data }: ActionsProps) {
  const { executeServerAction: executeDeleteCard, isLoading: isLoadingDelete } =
    useServerAction(deleteCard);
...
feat: Define prop types for AuditLog on creation
...
feat: Add authentication to audit log creation
...
Finally, return a `NextResponse` containing the audit log data in JSON format.

feat(api): Return audit log data for specific card

This commit implements the API route that fetches the audit log data for a specific card using the provided card ID.
...
feat: Create skeleton for Activity component
...
export default function ActivityItem({ data }: ActivityItemProps) {
  return (
    <li>
      ...
      <div>
        <p>
          <span>{data.userName}</span> {generateLogMessage(data)}
        </p>
      </div>
    </li>
  );
  ...
  return (
    <div className="flex items-start w-full gap-x-3">
      ...
      <div className="w-full">
        ...
        <ol className="mt-2 space-y-4">
          {data.map((auditLog: AuditLog) => (
            <ActivityItem data={auditLog} key={auditLog.id} />
          ))}
        </ol>
      </div>
    </div>
...
#### createBoard - audit log

feat: Integrate audit log for createBoard action
...
#### createList - audit log

feat: Integrate audit log for createList action

This commit adds audit logging to the createList server action. Whenever the action is executed, relevant audit data is captured, providing visibility into this essential operation. By monitoring user actions throughout the project, it improves workflow through increased collaboration, traceability, and security.
...
#### deleteBoard - audit log

feat: Integrate audit log for deleteBoard action
...
#### deleteCard - audit log

feat: Integrate audit log for deleteCard action

This commit adds audit logging to the deleteCard server action. Whenever the action is executed, relevant audit data is captured, providing visibility into this essential operation. By monitoring user actions throughout the project, it improves workflow through increased collaboration, traceability, and security.
...
#### deleteList - audit log

feat: Integrate audit log for deleteList action

This commit adds audit logging to the deleteList server action. Whenever the action is executed, relevant audit data is captured, providing visibility into this essential operation. By monitoring user actions throughout the project, it improves workflow through increased collaboration, traceability, and security.
...
async function performAction(data: InputType): Promise<OutputType> {
   // ...
  try {
    // Delete the list in the database...
...
#### updateCard - audit log

feat: Integrate audit log for updateCard action
...
#### updateList - audit log

feat: Integrate audit log for updateList action

This commit adds audit logging to the updateList server action. Whenever the action is executed, relevant audit data is captured, providing visibility into this essential operation. By monitoring user actions throughout the project, it improves workflow through increased collaboration, traceability, and security.
...
export default async function UserIdPage() {
  return <div className="flex flex-col space-y-4">UserIdPage</div>;
}
```
...
feat: Migrate to clerkMiddleware

feat(middleware): Protect all routes except one
...
To increment the available board count we want to authenticate and extract the `orgId`, fetch the `orgLimit`, and update the `orgLimit`. Dependening on whether `orgLimit` already exists, create an `orgLimit` or increment an existing `orgLimit`.

feat: Increment available board count
...
Implement `decrementAvailableBoardCount`, but ensure that we always have a non-negative `orgLimit` count.

feat: Decrement available board count

Implements the function `decrementAvailableBoardCount()` while ensuring that the `orgLimit` remains non-negative. This facilitates the management of board limits within an organization.
...
feat: Add utility to check available board count
...
feat: Add utility to get remaining board slots
...
3. Rename `BoardCreationButton` to `BoardCreationInfo` to better reflect the component

refactor: Rename BoardCreationButton to BoardCreationInfo

This commit updates the component name from BoardCreationButton to BoardCreationInfo. The new name better reflects the component's purpose, which includes rendering text, a count display, and a tooltip. Additionally, all affected imports have been adjusted accordingly.

4. Render BoardCreationInfo directly inside BoardList

feat: Render board creation info within BoardList

refactor: Render board creation display directly

This commit moves the BoardCreationInfo component rendering directly into the BoardList component, improving code organization and clarity. Additionally, this change ensures that the FormPopover behaves as intended.
